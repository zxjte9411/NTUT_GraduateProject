...
point = "null"
url = 'http://zxjte9411.ddns.net:8000'
money = 0
...
def stockPoint(bot, update):
    if (stock_num == "0000"):
        ...
        try:
            ...
            bot.sendMessage(chat_id=update.message.chat_id, text="選擇指標", reply_markup = reply_markup)
...
def getStockPoint(bot,update):
    try:     
        ...
        text = {"stock_num": stock_num ,"tptype": point, "csrfmiddlewaretoken": token}
        respose = request.post(url + '/stockPoint', data = text)
        image = json.loads(respose.text)
        img = requests.get(url + image["img_url"])
        imagefile = io.BytesIO(img.content)
        bot.sendPhoto(chat_id = update.callback_query.message.chat_id, photo = imagefile)
...
def detail(bot,update):
    ...
    if (money == 0):
        ...
        reply_markup = InlineKeyboardMarkup(keyboard)     
        bot.sendMessage(chat_id=update.message.chat_id, text="請選擇投資金額", reply_markup = reply_markup)
        ...
        try:
            ...
            ,[InlineKeyboardButton("BR", callback_data='BR'),InlineKeyboardButton("RSI", callback_data='RSI')],[InlineKeyboardButton("MA", callback_data='MA'),InlineKeyboardButton("PSY", callback_data='PSY')]]
            reply_markup = InlineKeyboardMarkup(keyboard)          
            bot.sendMessage(chat_id=update.message.chat_id, text="選擇要查詢的指標交易紀錄", reply_markup = reply_markup)
...
def getDetail(bot,update):
    try:
        ...
        text = {"stock_num": stock_num, "tptype": point, "money": money, "bot": "1", "csrfmiddlewaretoken": token}
        respose = request.post(url + '/withdraw/detail', data = text)
        info = json.loads(respose.text)["details"][point]

        for i in info:
            txt +=  i["date"] + " 收盤價 " + str(i["close"]) + " 進行 " + i["type"] + "\n"
        bot.sendMessage(chat_id=update.callback_query.message.chat_id, text=point + " 指標交易紀錄如下 ")
        bot.sendMessage(chat_id=update.callback_query.message.chat_id, text=txt)
    except Exception as e:
            print(e)
...
def button(bot, update):
    ...
    if (isDetail == 0):
        ...
        keyboard = [[InlineKeyboardButton("KD", callback_data='KD'),InlineKeyboardButton("OBV", callback_data='OBV')],[InlineKeyboardButton("AR", callback_data='AR'),InlineKeyboardButton("DMI", callback_data='DMI')]
            ,[InlineKeyboardButton("BR", callback_data='BR'),InlineKeyboardButton("RSI", callback_data='RSI')],[InlineKeyboardButton("MA", callback_data='MA'),InlineKeyboardButton("PSY", callback_data='PSY')]]
        reply_markup = InlineKeyboardMarkup(keyboard)          
        bot.sendMessage(chat_id=update.callback_query.message.chat_id, text="選擇要查詢的指標交易紀錄", reply_markup = reply_markup)
